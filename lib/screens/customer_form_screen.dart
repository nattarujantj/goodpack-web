...
class CustomerFormScreen extends StatefulWidget {
  ...
  const CustomerFormScreen({Key? key, this.customer, this.customerId, this.duplicateId}) : super(key: key);
...
class _CustomerFormScreenState extends State<CustomerFormScreen> {
  ...
  void _initializeForm() {
    if (widget.customer != null) {
      ...
      _loadDuplicateCustomer();
    } else {
      // สร้างผู้ติดต่อหลักเริ่มต้นสำหรับลูกค้าใหม่
      _contacts = [Contact(name: '', phone: '', isDefault: true)];
      _bankAccounts = [];
    }
  }

  void _loadDuplicateCustomer() {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      final customer = customerProvider.getCustomerById(widget.duplicateId!);
      if (customer != null) {
        _populateFieldsForDuplicate(customer);
      }
    });
  }

  void _populateFieldsForDuplicate(Customer customer) {
    _companyNameController.text = '${customer.companyName} (สำเนา)';
    ...
    if (customer.contacts.isNotEmpty) {
      _contacts = customer.contacts.map((c) => Contact(
        name: c.name,
        phone: c.phone,
        isDefault: c.isDefault,
      )).toList();
    } else if (customer.contactName.isNotEmpty) {
      _contacts = [Contact(name: customer.contactName, phone: customer.phone, isDefault: true)];
    } else {
      _contacts = [Contact(name: '', phone: '', isDefault: true)];
    }
    
    // Copy bank accounts
    _bankAccounts = customer.bankAccounts.map((b) => CustomerBankAccount(
      bankName: b.bankName,
