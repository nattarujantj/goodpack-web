...
class QuotationFormScreen extends StatefulWidget {
  ...
  const QuotationFormScreen({Key? key, this.quotation, this.quotationId, this.duplicateId}) : super(key: key);
...
ชำระเงินมัดจำก่อนผลิต 50% และอีก 50% ก่อนจัดส่งสินค้า''';
  ...
  void _loadDuplicateQuotation() {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      final quotation = quotationProvider.getQuotationById(widget.duplicateId!);
      if (quotation != null) {
        _populateFormForDuplicate(quotation);
      }
    });
  }

  void _populateFormForDuplicate(Quotation quotation) {
    // Copy all fields except quotationCode (will be generated by server)
    _quotationDateController.text = _formatDate(DateTime.now()); // Use current date
    _validUntilController.text = _formatDate(DateTime.now().add(const Duration(days: 10)));
    ...
    // Copy items
    _quotationItems = quotation.items.map((item) => QuotationItem(
