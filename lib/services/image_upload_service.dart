...
class ImageUploadService {
  ...
  static Future<Map<String, dynamic>> deleteProductImage({
    ...
    try {
      print('Starting image deletion for product: $productId');
      ...
      if (response.statusCode == 200) {
        ...
        return {
          'success': false,
          'error': 'Delete failed with status: ${response.statusCode} - ${response.body}',
        };
      }
    } catch (e) {
  ...
  static String getImageUrl(String? imagePath) {
    ...
    // Images are served from /uploads, not /api
    final baseUrl = EnvConfig.apiUrl.replaceAll('/api', '');
